...
/*
 * humanoid_robot_intelligence_control_system_controller.cpp
 *
...
RobotisController::RobotisController()
    : is_timer_running_(false), is_offset_enabled_(true), offset_ratio_(1.0),
      stop_timer_(false), init_pose_loaded_(false), timer_thread_(0),
      controller_mode_(MotionModuleMode), DEBUG_PRINT(false), robot_(0),
      gazebo_mode_(false),
      gazebo_robot_name_("humanoid_robot_intelligence_control_system") {
...
void RobotisController::initializeSyncWrite() {
  ...
  for (auto &it : robot_->dxls_) {
    ...
    for (int i = 0; i < dxl->bulk_read_items_.size(); i++) {
      ...
      if (port_to_bulk_read_[dxl->port_name_]->isAvailable(
              dxl->id_, dxl->bulk_read_items_[i]->address_,
              dxl->bulk_read_items_[i]->data_length_) == true) {
        read_data = port_to_bulk_read_[dxl->port_name_]->getData(
            dxl->id_, dxl->bulk_read_items_[i]->address_,
...
bool RobotisController::initialize(const std::string robot_file_path,
  ...
  if (gazebo_mode_ == true) {
    queue_thread_ =
        boost::thread(boost::bind(&RobotisController::msgQueueThread, this));
    return true;
  }
  ...
  for (auto &it : robot_->ports_) {
    ...
    dynamixel::PacketHandler *default_pkt_handler =
        dynamixel::PacketHandler::getPacketHandler(2.0);

    if (port->setBaudRate(port->getBaudRate()) == false) {
      ROS_ERROR("PORT [%s] SETUP ERROR! (baudrate: %d)", port_name.c_str(),
                port->getBaudRate());
      exit(-1);
    }
    ...
    if (dxl_it != robot_->dxls_.end()) {
      ...
      default_pkt_handler = dynamixel::PacketHandler::getPacketHandler(
          default_device->protocol_version_);

      if (default_device->goal_position_item_ != 0) {
        port_to_sync_write_position_[port_name] = new dynamixel::GroupSyncWrite(
            port, default_pkt_handler,
      ...
      if (default_device->goal_velocity_item_ != 0) {
        port_to_sync_write_velocity_[port_name] = new dynamixel::GroupSyncWrite(
            port, default_pkt_handler,
      ...
      if (default_device->goal_current_item_ != 0) {
        port_to_sync_write_current_[port_name] = new dynamixel::GroupSyncWrite(
            port, default_pkt_handler,
      ...
      default_pkt_handler = dynamixel::PacketHandler::getPacketHandler(
          _default_device->protocol_version_);
    }

    port_to_bulk_read_[port_name] =
        new dynamixel::GroupBulkRead(port, default_pkt_handler);
  }
...
void RobotisController::initializeDevice(const std::string init_file_path) {
  ...
  try {
    ...
    for (YAML::const_iterator it_doc = doc.begin(); it_doc != doc.end();
         it_doc++) {
      ...
      read1Byte(joint_name, dxl->torque_enable_item_->address_,
                &torque_enabled);

      for (YAML::const_iterator it_joint = joint_node.begin();
           it_joint != joint_node.end(); it_joint++) {
  ...
  for (auto &it : robot_->dxls_) {
      ...
      if (dxl->bulk_read_items_.size() != 0) {
        ...
        for (int i = 0; i < dxl->bulk_read_items_.size(); i++) {
          ...
          for (int l = 0; l < addr_leng; l++) {
            ...
            if (data16 != dxl->ctrl_table_[dxl->bulk_read_items_[i]->item_name_]
                                  ->address_ +
                              l) {
              if (torque_enabled == 1) {
              ...
              write2Byte(joint_name, indirect_addr,
                         dxl->ctrl_table_[dxl->bulk_read_items_[i]->item_name_]
                                 ->address_ +
                             l);
            }
            indirect_addr += 2;
    ...
    } else // INDIRECT_ADDRESS_1 NOT exist
    {
      if (dxl->bulk_read_items_.size() != 0) {
        ...
        bulkread_data_length =
            last_item->address_ - bulkread_start_addr + last_item->data_length_;
      }
    }
    ...
    if (bulkread_start_addr != 0)
      port_to_bulk_read_[dxl->port_name_]->addParam(
          dxl->id_, bulkread_start_addr, bulkread_data_length);

    // Torque ON
    if (writeCtrlItem(joint_name, dxl->torque_enable_item_->item_name_, 1) !=
        COMM_SUCCESS)
  ...
  for (auto &it : robot_->sensors_) {
      ...
      if (sensor->bulk_read_items_.size() != 0) {
        ...
        for (int i = 0; i < sensor->bulk_read_items_.size(); i++) {
          ...
          for (int l = 0; l < addr_leng; l++) {
            ...
            if (data16 !=
                sensor->ctrl_table_[sensor->bulk_read_items_[i]->item_name_]
                        ->address_ +
                    l) {
              write2Byte(
                  sensor_name, indirect_addr,
                  sensor->ctrl_table_[sensor->bulk_read_items_[i]->item_name_]
                          ->address_ +
                      l);
            }
    ...
    // bulkread_start_addr, bulkread_data_length);
    if (bulkread_start_addr != 0)
      port_to_bulk_read_[sensor->port_name_]->addParam(
          sensor->id_, bulkread_start_addr, bulkread_data_length);
  }
}
...
void RobotisController::msgQueueThread() {
  ...
  /* subscriber */
  ros::Subscriber write_control_table_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/write_control_table", 5,
      &RobotisController::writeControlTableCallback, this);
  ros::Subscriber sync_write_item_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/sync_write_item", 10,
      &RobotisController::syncWriteItemCallback, this);
  ros::Subscriber joint_ctrl_modules_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/set_joint_ctrl_modules", 10,
      &RobotisController::setJointCtrlModuleCallback, this);
  ros::Subscriber enable_ctrl_module_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/enable_ctrl_module", 10,
      &RobotisController::setCtrlModuleCallback, this);
  ros::Subscriber control_mode_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/set_control_mode", 10,
      &RobotisController::setControllerModeCallback, this);
  ros::Subscriber joint_states_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/set_joint_states", 10,
      &RobotisController::setJointStatesCallback, this);
  ros::Subscriber enable_offset_sub = ros_node.subscribe(
      "/humanoid_robot_intelligence_control_system/enable_offset", 10,
      &RobotisController::enableOffsetCallback, this);

  ros::Subscriber gazebo_joint_states_sub;
  if (gazebo_mode_ == true)
    gazebo_joint_states_sub =
        ros_node.subscribe("/" + gazebo_robot_name_ + "/joint_states", 10,
                           &RobotisController::gazeboJointStatesCallback, this);

  /* publisher */
  goal_joint_state_pub_ = ros_node.advertise<sensor_msgs::JointState>(
      "/humanoid_robot_intelligence_control_system/goal_joint_states", 10);
  present_joint_state_pub_ = ros_node.advertise<sensor_msgs::JointState>(
      "/humanoid_robot_intelligence_control_system/present_joint_states", 10);
  current_module_pub_ = ros_node.advertise<
      humanoid_robot_intelligence_control_system_controller_msgs::
          JointCtrlModule>(
      "/humanoid_robot_intelligence_control_system/present_joint_ctrl_modules",
      10);

  if (gazebo_mode_ == true) {
    for (auto &it : robot_->dxls_) {
      gazebo_joint_position_pub_[it.first] =
          ros_node.advertise<std_msgs::Float64>(
              "/" + gazebo_robot_name_ + "/" + it.first + "_position/command",
              1);
      gazebo_joint_velocity_pub_[it.first] =
          ros_node.advertise<std_msgs::Float64>(
              "/" + gazebo_robot_name_ + "/" + it.first + "_velocity/command",
              1);
  ...
  ros::ServiceServer set_joint_module_server = ros_node.advertiseService(
      "/humanoid_robot_intelligence_control_system/"
      "set_present_joint_ctrl_modules",
      &RobotisController::setJointCtrlModuleService, this);
  ros::ServiceServer set_module_server = ros_node.advertiseService(
      "/humanoid_robot_intelligence_control_system/set_present_ctrl_modules",
      &RobotisController::setCtrlModuleService, this);
  ros::ServiceServer load_offset_server = ros_node.advertiseService(
      "/humanoid_robot_intelligence_control_system/load_offset",
...
void *RobotisController::timerThread(void *param) {
  ...
  while (!controller->stop_timer_) {
    next_time.tv_sec +=
        (next_time.tv_nsec + controller->robot_->getControlCycle() * 1000000) /
        1000000000;
    next_time.tv_nsec =
        (next_time.tv_nsec + controller->robot_->getControlCycle() * 1000000) %
        1000000000;
    ...
    long delta_nsec = (next_time.tv_sec - curr_time.tv_sec) * 1000000000 +
                      (next_time.tv_nsec - curr_time.tv_nsec);
    if (delta_nsec < -100000) {
      if (controller->DEBUG_PRINT == true) {
        fprintf(stderr,
                "[RobotisController::ThreadProc] NEXT TIME < CURR TIME.. "
                "(%f)[%ld.%09ld / %ld.%09ld]",
                delta_nsec / 1000000.0, (long)next_time.tv_sec,
                (long)next_time.tv_nsec, (long)curr_time.tv_sec,
                (long)curr_time.tv_nsec);
      }

      // next_time = curr_time + 3 msec
      next_time.tv_sec =
          curr_time.tv_sec + (curr_time.tv_nsec + 3000000) / 1000000000;
...
void RobotisController::startTimer() {
  ...
  if (this->gazebo_mode_ == true) {
    ...
    gazebo_thread_ =
        boost::thread(boost::bind(&RobotisController::gazeboTimerThread, this));
  } else {
    initializeSyncWrite();
    ...
    // create and start the thread
    if ((error = pthread_create(&this->timer_thread_, &attr, this->timerThread,
                                this)) != 0) {
...
bool RobotisController::isTimerRunning() { return this->is_timer_running_; }
...
void RobotisController::loadOffset(const std::string path) {
  ...
  for (YAML::const_iterator it = offset_node.begin(); it != offset_node.end();
       it++) {
...
void RobotisController::process() {
  ...
  if (controller_mode_ == MotionModuleMode) {
    if (gazebo_mode_ == false) {
        ...
        for (auto &dxl_it : robot_->dxls_) {
          ...
          if (dxl->bulk_read_items_.size() > 0) {
            ...
            for (int i = 0; i < dxl->bulk_read_items_.size(); i++) {
              ...
              if (port_to_bulk_read_[port_name]->isAvailable(
                      dxl->id_, item->address_, item->data_length_) == true) {
                updated = true;
                data = port_to_bulk_read_[port_name]->getData(
                    dxl->id_, item->address_, item->data_length_);
                ...
                if (dxl->present_position_item_ != 0 &&
                    item->item_name_ ==
                        dxl->present_position_item_->item_name_) {
                  dxl->dxl_state_->present_position_ =
                      dxl->convertValue2Radian(data) -
                      dxl->dxl_state_->position_offset_ * offset_ratio_;
                } else if (dxl->present_velocity_item_ != 0 &&
                           item->item_name_ ==
                               dxl->present_velocity_item_->item_name_)
                  dxl->dxl_state_->present_velocity_ =
                      dxl->convertValue2Velocity(data);
                else if (dxl->present_current_item_ != 0 &&
                         item->item_name_ ==
                             dxl->present_current_item_->item_name_)
                  dxl->dxl_state_->present_torque_ =
                      dxl->convertValue2Torque(data);
                else if (dxl->goal_position_item_ != 0 &&
                         item->item_name_ ==
                             dxl->goal_position_item_->item_name_) {
                  dxl->dxl_state_->goal_position_ =
                      dxl->convertValue2Radian(data) -
                      dxl->dxl_state_->position_offset_ * offset_ratio_;
                } else if (dxl->goal_velocity_item_ != 0 &&
                           item->item_name_ ==
                               dxl->goal_velocity_item_->item_name_)
                  dxl->dxl_state_->goal_velocity_ =
                      dxl->convertValue2Velocity(data);
                else if (dxl->goal_current_item_ != 0 &&
                         item->item_name_ ==
                             dxl->goal_current_item_->item_name_)
                  dxl->dxl_state_->goal_torque_ =
                      dxl->convertValue2Torque(data);
            ...
            if (updated == true)
              dxl->dxl_state_->update_time_stamp_ =
                  TimeStamp(present_state.header.stamp.sec,
                            present_state.header.stamp.nsec);
          }
        }
      }
        ...
        for (auto &sensor_it : robot_->sensors_) {
          ...
          if (sensor->bulk_read_items_.size() > 0) {
            ...
            for (int i = 0; i < sensor->bulk_read_items_.size(); i++) {
              ...
              if (port_to_bulk_read_[port_name]->isAvailable(
                      sensor->id_, item->address_, item->data_length_) ==
                  true) {
                updated = true;
                data = port_to_bulk_read_[port_name]->getData(
                    sensor->id_, item->address_, item->data_length_);
                ...
                // change sensor_state
                sensor->sensor_state_->bulk_read_table_[item->item_name_] =
                    data;
              }
            }
      ...
      for (auto module_it = motion_modules_.begin();
           module_it != motion_modules_.end(); module_it++) {
  ...
  if (sensor_modules_.size() > 0) {
    for (auto module_it = sensor_modules_.begin();
         module_it != sensor_modules_.end(); module_it++) {
        ...
        for (auto &dxl_it : robot_->dxls_) {
          ...
          if (dxl->ctrl_module_name_ == (*module_it)->getModuleName()) {
            ...
            if (result_state == NULL) {
              ROS_ERROR("[%s] %s ", (*module_it)->getModuleName().c_str(),
                        joint_name.c_str());
              continue;
            }
            ...
            if ((*module_it)->getControlMode() == PositionControl) {
              ...
              if (gazebo_mode_ == false) {
                // add offset
                uint32_t pos_data;
                pos_data = dxl->convertRadian2Value(
                    dxl_state->goal_position_ +
                    dxl_state->position_offset_ * offset_ratio_);
                ...
                if (port_to_sync_write_position_[dxl->port_name_] != NULL)
                  port_to_sync_write_position_[dxl->port_name_]->changeParam(
                      dxl->id_, sync_write_data);

                // if position p gain value is changed -> sync write
                if (result_state->position_p_gain_ != NONE_GAIN &&
                    dxl_state->position_p_gain_ !=
                        result_state->position_p_gain_) {
                  dxl_state->position_p_gain_ = result_state->position_p_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->position_p_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->position_p_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->position_p_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->position_p_gain_));

                  if (port_to_sync_write_position_p_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_position_p_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }

                // if position i gain value is changed -> sync write
                if (result_state->position_i_gain_ != NONE_GAIN &&
                    dxl_state->position_i_gain_ !=
                        result_state->position_i_gain_) {
                  dxl_state->position_i_gain_ = result_state->position_i_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->position_i_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->position_i_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->position_i_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->position_i_gain_));

                  if (port_to_sync_write_position_i_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_position_i_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }

                // if position d gain value is changed -> sync write
                if (result_state->position_d_gain_ != NONE_GAIN &&
                    dxl_state->position_d_gain_ !=
                        result_state->position_d_gain_) {
                  dxl_state->position_d_gain_ = result_state->position_d_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->position_d_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->position_d_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->position_d_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->position_d_gain_));

                  if (port_to_sync_write_position_d_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_position_d_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }

                // if velocity p gain gain value is changed -> sync write
                if (result_state->velocity_p_gain_ != NONE_GAIN &&
                    dxl_state->velocity_p_gain_ !=
                        result_state->velocity_p_gain_) {
                  dxl_state->velocity_p_gain_ = result_state->velocity_p_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->velocity_p_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->velocity_p_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->velocity_p_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->velocity_p_gain_));

                  if (port_to_sync_write_velocity_p_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_velocity_p_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }

                // if velocity i gain value is changed -> sync write
                if (result_state->velocity_i_gain_ != NONE_GAIN &&
                    dxl_state->velocity_i_gain_ !=
                        result_state->velocity_i_gain_) {
                  dxl_state->velocity_i_gain_ = result_state->velocity_i_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->velocity_i_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->velocity_i_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->velocity_i_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->velocity_i_gain_));

                  if (port_to_sync_write_velocity_i_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_velocity_i_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }
              }
            } else if ((*module_it)->getControlMode() == VelocityControl) {
              ...
              if (gazebo_mode_ == false) {
                uint32_t vel_data =
                    dxl->convertVelocity2Value(dxl_state->goal_velocity_);
                uint8_t sync_write_data[4] = {0};
                ...
                if (port_to_sync_write_velocity_[dxl->port_name_] != NULL)
                  port_to_sync_write_velocity_[dxl->port_name_]->changeParam(
                      dxl->id_, sync_write_data);
                ...
                if (result_state->velocity_d_gain_ != NONE_GAIN &&
                    dxl_state->velocity_d_gain_ !=
                        result_state->velocity_d_gain_) {
                  dxl_state->velocity_d_gain_ = result_state->velocity_d_gain_;
                  uint8_t sync_write_data[4] = {0};
                  sync_write_data[0] =
                      DXL_LOBYTE(DXL_LOWORD(dxl_state->velocity_d_gain_));
                  sync_write_data[1] =
                      DXL_HIBYTE(DXL_LOWORD(dxl_state->velocity_d_gain_));
                  sync_write_data[2] =
                      DXL_LOBYTE(DXL_HIWORD(dxl_state->velocity_d_gain_));
                  sync_write_data[3] =
                      DXL_HIBYTE(DXL_HIWORD(dxl_state->velocity_d_gain_));

                  if (port_to_sync_write_velocity_d_gain_[dxl->port_name_] !=
                      NULL)
                    port_to_sync_write_velocity_d_gain_[dxl->port_name_]
                        ->addParam(dxl->id_, sync_write_data);
                }
              }
            } else if ((*module_it)->getControlMode() == TorqueControl) {
              ...
              if (gazebo_mode_ == false) {
                uint32_t curr_data =
                    dxl->convertTorque2Value(dxl_state->goal_torque_);
                ...
                if (port_to_sync_write_current_[dxl->port_name_] != NULL)
                  port_to_sync_write_current_[dxl->port_name_]->changeParam(
                      dxl->id_, sync_write_data);
              }
            }
          }
...
void RobotisController::addMotionModule(MotionModule *module) {
  ...
  for (auto m_it = motion_modules_.begin(); m_it != motion_modules_.end();
       m_it++) {
    if ((*m_it)->getModuleName() == module->getModuleName()) {
      ROS_ERROR("Motion Module Name [%s] already exist !!",
...
void RobotisController::addSensorModule(SensorModule *module) {
  // check whether the module name already exists
  for (auto m_it = sensor_modules_.begin(); m_it != sensor_modules_.end();
       m_it++) {
    if ((*m_it)->getModuleName() == module->getModuleName()) {
      ROS_ERROR("Sensor Module Name [%s] already exist !!",
                module->getModuleName().c_str());
      return;
    }
  }
...
void RobotisController::writeControlTableCallback(
    const humanoid_robot_intelligence_control_system_controller_msgs::
        WriteControlTable::ConstPtr &msg) {
  Device *device = NULL;
  ...
  if (dev_it1 != robot_->dxls_.end()) {
    ...
    if (dev_it2 != robot_->sensors_.end()) {
      ...
      ROS_WARN("[WriteControlTable] Unknown device : %s",
  ...
  if (item_it != device->ctrl_table_.end()) {
    ...
    ROS_WARN("[WriteControlTable] Unknown item : %s",
             msg->start_item_name.c_str());
    return;
  }

  dynamixel::PortHandler *port = robot_->ports_[device->port_name_];
  dynamixel::PacketHandler *packet_handler =
      dynamixel::PacketHandler::getPacketHandler(device->protocol_version_);
  ...
  direct_sync_write_.push_back(new dynamixel::GroupSyncWrite(
      port, packet_handler, item->address_, msg->data_length));
  direct_sync_write_[direct_sync_write_.size() - 1]->addParam(
      device->id_, (uint8_t *)(msg->data.data()));
...
void RobotisController::syncWriteItemCallback(
    const humanoid_robot_intelligence_control_system_controller_msgs::
        SyncWriteItem::ConstPtr &msg) {
  for (int i = 0; i < msg->joint_name.size(); i++) {
    ...
    if (d_it1 != robot_->dxls_.end()) {
      ...
      if (d_it2 != robot_->sensors_.end()) {
        ...
        ROS_WARN("[SyncWriteItem] Unknown device : %s",
    ...
    if (direct_sync_write_.size() == 0) {
      direct_sync_write_.push_back(new dynamixel::GroupSyncWrite(
          port, packet_handler, item->address_, item->data_length_));
      idx = 0;
    } else {
...
void RobotisController::setControllerModeCallback(
...
void RobotisController::setJointStatesCallback(
    ...
    if ((controller_mode_ == DirectControlMode) ||
      ...
      if (gazebo_mode_ == false) {
        ...
        pos_data = dxl->convertRadian2Value(dxl->dxl_state_->goal_position_ +
                                            dxl->dxl_state_->position_offset_ *
                                                offset_ratio_);
...
void RobotisController::setCtrlModuleCallback(
    const std_msgs::String::ConstPtr &msg) {
  ...
  set_module_thread_ = boost::thread(boost::bind(
      &RobotisController::setCtrlModuleThread, this, _module_name_to_set));
}
...
void RobotisController::setCtrlModule(std::string module_name) {
  ...
  set_module_thread_ = boost::thread(
      boost::bind(&RobotisController::setCtrlModuleThread, this, module_name));
}
void RobotisController::setJointCtrlModuleCallback(
    const humanoid_robot_intelligence_control_system_controller_msgs::
        JointCtrlModule::ConstPtr &msg) {
  ...
  set_module_thread_ = boost::thread(
      boost::bind(&RobotisController::setJointCtrlModuleThread, this, msg));
}

void RobotisController::enableOffsetCallback(
...
bool RobotisController::getJointCtrlModuleService(
    humanoid_robot_intelligence_control_system_controller_msgs::GetJointModule::
        Request &req,
    humanoid_robot_intelligence_control_system_controller_msgs::GetJointModule::
        Response &res) {
...
bool RobotisController::setJointCtrlModuleService(
    humanoid_robot_intelligence_control_system_controller_msgs::SetJointModule::
        Request &req,
    humanoid_robot_intelligence_control_system_controller_msgs::SetJointModule::
        Response &res) {
  if (set_module_thread_.joinable())
  ...
  humanoid_robot_intelligence_control_system_controller_msgs::JointCtrlModule
      modules;
  ...
  humanoid_robot_intelligence_control_system_controller_msgs::JointCtrlModule::
      ConstPtr msg_ptr(
          new humanoid_robot_intelligence_control_system_controller_msgs::
              JointCtrlModule(modules));

  if (modules.joint_name.size() != modules.module_name.size())
    return false;

  set_module_thread_ = boost::thread(
      boost::bind(&RobotisController::setJointCtrlModuleThread, this, msg_ptr));
...
bool RobotisController::setCtrlModuleService(
    humanoid_robot_intelligence_control_system_controller_msgs::SetModule::
        Request &req,
    humanoid_robot_intelligence_control_system_controller_msgs::SetModule::
        Response &res) {
  if (set_module_thread_.joinable())
...
bool RobotisController::loadOffsetService(
    humanoid_robot_intelligence_control_system_controller_msgs::LoadOffset::
        Request &req,
    humanoid_robot_intelligence_control_system_controller_msgs::LoadOffset::
        Response &res) {
...
void RobotisController::setJointCtrlModuleThread(
    const humanoid_robot_intelligence_control_system_controller_msgs::
        JointCtrlModule::ConstPtr &msg) {
  // stop module list
  ...
  for (unsigned int idx = 0; idx < msg->joint_name.size(); idx++) {
    Dynamixel *_dxl = NULL;
    std::map<std::string, Dynamixel *>::iterator _dxl_it =
        robot_->dxls_.find((std::string)(msg->joint_name[idx]));
    ...
    if (_dxl->ctrl_module_name_ != msg->module_name[idx]) {
      for (std::list<MotionModule *>::iterator _stop_m_it =
               motion_modules_.begin();
           _stop_m_it != motion_modules_.end(); _stop_m_it++) {
        if ((*_stop_m_it)->getModuleName() == _dxl->ctrl_module_name_ &&
            (*_stop_m_it)->getModuleEnable() == true)
  ...
  for (std::list<MotionModule *>::iterator _stop_m_it = _stop_modules.begin();
       _stop_m_it != _stop_modules.end(); _stop_m_it++) {
    (*_stop_m_it)->stop();
  }
  ...
  for (unsigned int idx = 0; idx < msg->joint_name.size(); idx++) {
    ...
    if (ctrl_module == "" || ctrl_module == "none") {
      ...
      _pos_data = _dxl->convertRadian2Value(_dxl->dxl_state_->goal_position_ +
                                            _dxl->dxl_state_->position_offset_ *
                                                offset_ratio_);
      ...
      if (port_to_sync_write_position_[_dxl->port_name_] != NULL)
        port_to_sync_write_position_[_dxl->port_name_]->addParam(
            _dxl->id_, _sync_write_data);
      ...
      for (std::list<MotionModule *>::iterator _m_it = motion_modules_.begin();
           _m_it != motion_modules_.end(); _m_it++) {
        // if it exist
        if ((*_m_it)->getModuleName() == ctrl_module) {
          std::map<std::string, DynamixelState *>::iterator _result_it =
              (*_m_it)->result_.find(joint_name);
          ...
          if (_mode == PositionControl) {
            uint32_t _pos_data;
            _pos_data = _dxl->convertRadian2Value(
                _dxl->dxl_state_->goal_position_ +
                _dxl->dxl_state_->position_offset_ * offset_ratio_);
            ...
            if (port_to_sync_write_current_[_dxl->port_name_] != NULL)
              port_to_sync_write_current_[_dxl->port_name_]->removeParam(
                  _dxl->id_);
            if (port_to_sync_write_velocity_[_dxl->port_name_] != NULL)
              port_to_sync_write_velocity_[_dxl->port_name_]->removeParam(
                  _dxl->id_);
          } else if (_mode == VelocityControl) {
            uint32_t _vel_data =
                _dxl->convertVelocity2Value(_dxl->dxl_state_->goal_velocity_);
            uint8_t _sync_write_data[4];
            ...
            if (port_to_sync_write_velocity_[_dxl->port_name_] != NULL)
              port_to_sync_write_velocity_[_dxl->port_name_]->addParam(
                  _dxl->id_, _sync_write_data);
            ...
            if (port_to_sync_write_current_[_dxl->port_name_] != NULL)
              port_to_sync_write_current_[_dxl->port_name_]->removeParam(
                  _dxl->id_);
            if (port_to_sync_write_position_[_dxl->port_name_] != NULL)
              port_to_sync_write_position_[_dxl->port_name_]->removeParam(
                  _dxl->id_);
          } else if (_mode == TorqueControl) {
            uint32_t _curr_data =
                _dxl->convertTorque2Value(_dxl->dxl_state_->goal_torque_);
            uint8_t _sync_write_data[4];
            ...
            if (port_to_sync_write_current_[_dxl->port_name_] != NULL)
              port_to_sync_write_current_[_dxl->port_name_]->addParam(
                  _dxl->id_, _sync_write_data);
  ...
  for (std::list<MotionModule *>::iterator _m_it = _enable_modules.begin();
       _m_it != _enable_modules.end(); _m_it++) {
  ...
  humanoid_robot_intelligence_control_system_controller_msgs::JointCtrlModule
      _current_module_msg;
  for (std::map<std::string, Dynamixel *>::iterator _dxl_iter =
           robot_->dxls_.begin();
       _dxl_iter != robot_->dxls_.end(); ++_dxl_iter) {
    _current_module_msg.joint_name.push_back(_dxl_iter->first);
    _current_module_msg.module_name.push_back(
        _dxl_iter->second->ctrl_module_name_);
  }

  if (_current_module_msg.joint_name.size() ==
      _current_module_msg.module_name.size())
...
void RobotisController::setCtrlModuleThread(std::string ctrl_module) {
        ...
        for (auto &result_it : (*m_it)->result_) {
          ...
          if (d_it != robot_->dxls_.end()) {
            ...
            if (d_it->second->ctrl_module_name_ != ctrl_module) {
              for (auto stop_m_it = motion_modules_.begin();
                   stop_m_it != motion_modules_.end(); stop_m_it++) {
                if (((*stop_m_it)->getModuleName() ==
                     d_it->second->ctrl_module_name_) &&
  ...
  // stop the module
  stop_modules.unique();
  for (auto stop_m_it = stop_modules.begin(); stop_m_it != stop_modules.end();
       stop_m_it++) {
    (*stop_m_it)->stop();
  }
  ...
  // disable module(s)
  for (std::list<MotionModule *>::iterator _stop_m_it = stop_modules.begin();
       _stop_m_it != stop_modules.end(); _stop_m_it++) {
    (*_stop_m_it)->setModuleEnable(false);
  }
        ...
        for (auto &result_it : (*m_it)->result_) {
          ...
          if (d_it != robot_->dxls_.end()) {
            ...
            if (mode == PositionControl) {
              ...
              if (port_to_sync_write_current_[dxl->port_name_] != NULL)
                port_to_sync_write_current_[dxl->port_name_]->removeParam(
                    dxl->id_);
              if (port_to_sync_write_velocity_[dxl->port_name_] != NULL)
                port_to_sync_write_velocity_[dxl->port_name_]->removeParam(
                    dxl->id_);
            } else if (mode == VelocityControl) {
              uint32_t vel_data =
                  dxl->convertVelocity2Value(dxl->dxl_state_->goal_velocity_);
              uint8_t sync_write_data[4] = {0};
              sync_write_data[0] = DXL_LOBYTE(DXL_LOWORD(vel_data));
              sync_write_data[1] = DXL_HIBYTE(DXL_LOWORD(vel_data));
              sync_write_data[2] = DXL_LOBYTE(DXL_HIWORD(vel_data));
              ...
              if (port_to_sync_write_velocity_[dxl->port_name_] != NULL)
                port_to_sync_write_velocity_[dxl->port_name_]->addParam(
                    dxl->id_, sync_write_data);
              ...
              if (port_to_sync_write_current_[dxl->port_name_] != NULL)
                port_to_sync_write_current_[dxl->port_name_]->removeParam(
                    dxl->id_);
              if (port_to_sync_write_position_[dxl->port_name_] != NULL)
                port_to_sync_write_position_[dxl->port_name_]->removeParam(
                    dxl->id_);
            } else if (mode == TorqueControl) {
              uint32_t curr_data =
                  dxl->convertTorque2Value(dxl->dxl_state_->goal_torque_);
              uint8_t sync_write_data[4] = {0};
              ...
              if (port_to_sync_write_current_[dxl->port_name_] != NULL)
                port_to_sync_write_current_[dxl->port_name_]->addParam(
                    dxl->id_, sync_write_data);
  ...
  for (auto &dxl_iter : robot_->dxls_) {
    ...
    current_module_msg.module_name.push_back(
        dxl_iter.second->ctrl_module_name_);
  }

  if (current_module_msg.joint_name.size() ==
      current_module_msg.module_name.size())
    current_module_pub_.publish(current_module_msg);
}

void RobotisController::gazeboJointStatesCallback(
    const sensor_msgs::JointState::ConstPtr &msg) {
  queue_mutex_.lock();
  ...
  if (init_pose_loaded_ == false) {
    for (auto &it : robot_->dxls_)
      it.second->dxl_state_->goal_position_ =
          it.second->dxl_state_->present_position_;
    init_pose_loaded_ = true;
  }
...
int RobotisController::ping(const std::string joint_name,
                            uint16_t *model_number, uint8_t *error) {
...
int RobotisController::read(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->readTxRx(port_handler, dxl->id_, address, length, data,
                               error);
}

int RobotisController::readCtrlItem(const std::string joint_name,
                                    const std::string item_name, uint32_t *data,
                                    uint8_t *error) {
  if (isTimerStopped() == false)
    return COMM_PORT_BUSY;
  ...
  case 1: {
    uint8_t read_data = 0;
    result = pkt_handler->read1ByteTxRx(port_handler, dxl->id_, item->address_,
                                        &read_data, error);
    if (result == COMM_SUCCESS)
      *data = read_data;
    break;
  ...
  case 2: {
    ...
    result = pkt_handler->read2ByteTxRx(port_handler, dxl->id_, item->address_,
                                        &read_data, error);
    if (result == COMM_SUCCESS)
      *data = read_data;
    break;
  ...
  case 4: {
    uint32_t read_data = 0;
    result = pkt_handler->read4ByteTxRx(port_handler, dxl->id_, item->address_,
                                        &read_data, error);
    if (result == COMM_SUCCESS)
      *data = read_data;
    break;
...
int RobotisController::read1Byte(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->read1ByteTxRx(port_handler, dxl->id_, address, data,
                                    error);
}
...
int RobotisController::read2Byte(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->read2ByteTxRx(port_handler, dxl->id_, address, data,
                                    error);
}
...
int RobotisController::read4Byte(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->read4ByteTxRx(port_handler, dxl->id_, address, data,
                                    error);
}
...
int RobotisController::write(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->writeTxRx(port_handler, dxl->id_, address, length, data,
                                error);
}
...
int RobotisController::writeCtrlItem(const std::string joint_name,
  ...
  if (item->data_length_ == 1) {
    ...
    result = pkt_handler->write1ByteTxRx(port_handler, dxl->id_, item->address_,
                                         data, error);
  } else if (item->data_length_ == 2) {
    write_data[0] = DXL_LOBYTE((uint16_t)data);
    write_data[1] = DXL_HIBYTE((uint16_t)data);
    result = pkt_handler->write2ByteTxRx(port_handler, dxl->id_, item->address_,
                                         data, error);
  } else if (item->data_length_ == 4) {
    ...
    result = pkt_handler->write4ByteTxRx(port_handler, dxl->id_, item->address_,
                                         data, error);
  }
  delete[] write_data;
  return result;
...
int RobotisController::write1Byte(const std::string joint_name,
  ...
  return pkt_handler->write1ByteTxRx(port_handler, dxl->id_, address, data,
                                     error);
}
...
int RobotisController::write2Byte(const std::string joint_name,
  ...
  return pkt_handler->write2ByteTxRx(port_handler, dxl->id_, address, data,
                                     error);
}
...
int RobotisController::write4Byte(const std::string joint_name,
  ...
  return pkt_handler->write4ByteTxRx(port_handler, dxl->id_, address, data,
                                     error);
}
...
int RobotisController::regWrite(const std::string joint_name, uint16_t address,
  ...
  return pkt_handler->regWriteTxRx(port_handler, dxl->id_, address, length,
                                   data, error);
}
